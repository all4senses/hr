 ...
 */
/*
function hr_preprocess_user_profile(&$profile) {
  //dpm($profile);
}
*/
 ...
 */
function hr_tablesort_indicator($variables) {
 ...
 */
function hr_preprocess_user_picture(&$picture) {
  ...
  if (!empty($picture['account']->realname)) {
    ...
    $userExtendedData = hr_misc_loadUserExtendedData($picture['account']->uid);
 ...
 * @ingroup themeable
 */
function hr_pager_next($variables) {
  ...
  if ($out) {
    ...
    if(preg_match('|.*href="([^"]*)" .*title="([^"]*)" .*|', $out_altered, $matches) && !empty($matches[1])) {
      ...
      if ($matches[2] == 'Go to next page') {
        hr_misc_addMetatag('next', NULL, $href = 'http://bestwebhosting.ca' . $matches[1]);
      }
    }
  }
  return $out;
 ...
 * @ingroup themeable
 */
function hr_pager_previous($variables) {
  ...
  if ($out) {
    ...
    if(preg_match('|.*href="([^"]*)" .*title="([^"]*)" .*|', $out_altered, $matches) && !empty($matches[1])) {
      ...
      if ($matches[2] == 'Go to previous page') {
        hr_misc_addMetatag('prev', NULL, $href = 'http://bestwebhosting.ca' . $matches[1]);
      }
...
//  }
//  elseif ( $arg_0 == 'node' && (in_array($arg_1, $altered_pager_reviews) || hr_misc_identifyNodeType($arg_1) == 'provider') ) {
...
/**
 * Implements hook_preprocess_searhr_results().
 */
function hr_preprocess_searhr_results(&$variables) {
  
//  // a4s - fix - show lost pager from the results page on some pages.
//  // need prior actions (hack) in function node_searhr_execute() at node.module
//  // v1
...
//  $variables['pager'] = theme('pager', array('tags' => NULL, 'element' => 7));
  ...
  // need prior actions (hack) in function node_searhr_execute() at node.module
  // v2
  global $hr_num_rows, $hr_limit_rows;
//  dpm('$hr_num_rows =' . $hr_num_rows, '$hr_limit_rows = ' . $hr_limit_rows);
//  dpm($_SESSION['hr_node_searhr_data']);
//  $page = pager_default_initialize($_SESSION['hr_node_searhr_data']['hr_num_rows'], $_SESSION['hr_node_searhr_data']['hr_limit_rows']);
  $page = pager_default_initialize($hr_num_rows, $hr_limit_rows);
 ...
 */
function hr_html_head_alter(&$head_elements) {
...
//      global $user;
//      if ($user->uid == 1) {
//        hr_misc_addMetatag('title', $altered_head_title);
//      }
    }
  }
 ...
 */
function hr_menu_local_tasks(&$variables) {
  $output = '';
...
/*
function hr_preprocess_breadcrumb(&$variables) {
  dpm($variables);
}
*/
 ...
 */
function hr_breadcrumb($variables) {
 ...
 */
function hr_process_page(&$variables) {
...
//  }
  ...
  // Set breadcrumb
  hr_misc_setBreadcrumbs($variables);
  ...
  hr_misc_addMetatag('stylesheet', NULL, 'http://fonts.googleapis.com/css?family=Open+Sans:normal,bold|Ubuntu:500|Roboto:500|Raleway:300,500', 'NA', 'text/css');
 ...
 */
function hr_preprocess_region(&$variables, $hook) {
 ...
 */
function hr_preprocess_block(&$variables, $hook) {
 ...
 *   The name of the template being rendered ("block" in this case.)
 */
function hr_process_block(&$variables, $hook) {
 ...
 */
function hr_preprocess_searhr_block_form(&$vars) {
  $vars['searhr_form'] = str_replace('type="text"', 'type="search"', $vars['searhr_form']);
}

/**
 * Changes the search button label. Here... because it doesn't work via module hook.
 */
function hr_form_alter(&$form, &$form_state, $form_id) {
  if ($form_id == 'searhr_block_form') {
    
    // Hide a word on the button "Search"
    ///$form['actions']['submit']['#value'] = '';
    
    // Autohiding hint.
    $default_searhr_text = t('Search on site');
    $form['searhr_block_form']['#default_value'] = $default_searhr_text;
    $form['default_text']['#default_value'] = $default_searhr_text;
    // Cause a fatal error without it for anonymous.
    $form['default_text']['#type'] = 'hidden';
    
    
    // Example! Does NOT work!!! The above one works!
    //$form['searhr_block_form']['#value'] = t($default_searhr_text); // Set a default value for the textfield
    
    // Add extra attributes to the text box
    // Without this a hint autohiding will not work for anonymous.
    $form['searhr_block_form']['#attributes']['onblur'] = "if (this.value == '') {this.value = '$default_searhr_text';}";
    $form['searhr_block_form']['#attributes']['onfocus'] = "if (this.value == '$default_searhr_text') {this.value = '';}";
  }
}


function hr_username($object) {
...
*/
function hr_preprocess_views_view_rss(&$vars) {
...
*/
function hr_preprocess_views_view_archive(&$vars) {
  dpr('xxxxzzzz');
  exit;
  //dpm($vars['view']);
}
...
* Default theme function for all RSS rows.
*/
function hr_preprocess_views_view_row(&$vars) {
  dpm('xxxxxx');
  exit;  
}
...
* Default theme function for all RSS rows.
*/
function hr_preprocess_views_view_row_rss(&$vars) {
  ...
  if (isset($node->body['und'][0]['value'])) {
    $rss_teaser = hr_misc_getArticleTeaserData('all', $node->body['und'][0]['value'], $vars['row']->nid, 400, TRUE);
  ...
  if (!empty($extra_data['guest_author'])) {
    ...
    $vars['item_elements'] = preg_replace('|<dc:creator>.*</dc:creator>|', '<dc:creator>' . hr_misc_getUserRealName($node->uid) . '</dc:creator>', $vars['item_elements']);
  }
...
function hr_preprocess_views_view_unformatted(&$vars) {
//  dpr('yyy');
//exit;
...
function hr_preprocess_views_view(&$vars) {
  //dpm($vars);
  //$vars['view']->rows = 'xxx';
  //$vars['rows'] = 'xxx';
  ...
  /*
  // I set title for preface (at hr_misc_views_pre_render(&$view)) instead of a view itself.
...
//  }
  ...
  if (isset($vars['view']->name) && ($vars['view']->name == 'blog' || $vars['view']->name == 'news') ) {
    $function = 'hr_preprocess_views_view_unformatted'; 
 ...
 * 3. Read carefully, especially within hr_preprocess_html(), there
 ...
 */
function hr_preprocess_html(&$vars) {
  // Load the media queries styles
  // Remember to rename these files to match the names used here - they are
  // in the CSS directory of your subtheme.
//  $media_queries_css = array(
//    'hr.responsive.style.css',
//    'hr.responsive.gpanels.css'
//  );
//  load_subtheme_media_queries($media_queries_css, 'hr');
...
//  );
//  load_subtheme_ie_styles($ie_files, 'hr');
 ...
 * @see comment.tpl.php
 */
function hr_preprocess_comment(&$variables) {
...
/*
  ...
  // Preprocess fields.
  field_attahr_preprocess('comment', $comment, $variables['elements'], $variables);
